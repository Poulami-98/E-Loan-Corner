...
<body>
	...
	<br/>
		...
		<br/>
		<h4><a href="/acceptCondition">Click here</a><i> to find the necessary conditions for accepting loan.</i></h4>
	    <br/>
	<table class="table table-bordered">
				...
				<thead>
					<tr class="success">
						...
						<th>Loan Amount</th>
						<th>Annual Income</th>
						<th>Percentage of Annual Income(%)</th>
						<th>Loan Status</th>
				...
				<tbody>
					<c:forEach items="${custdetails}" var="cust">
						<tr class="success">
							<td>${cust.id}</td>
							<td>${cust.name}</td>
							<td>${cust.phone_no}</td>
							<td>${cust.loan_type}</td>
							<td>${cust.loan_amount}</td>
							<td>${cust.annual_income}</td>
							<td>${cust.perc}</td>
							<td>${cust.loan_status}</td>
							<td>
								<a type="button" class="btn btn-primary" 
									href="/approve?id=${cust.id}">Approve</a>
	
								<a type="button" class="btn btn-warning" 
									href="/reject?id=${cust.id}">Reject</a>
							</td>
						</tr>
					</c:forEach>
				</tbody>
			...
			<a class="btn btn-success" href="/admin" role="button">back</a>
				</p>
